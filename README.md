...
Hallucinations are really tricky but there are a couple strategies I would use to deter them. The first is by enforcing that the evidence for each code is verbatim so it can be verified and the selected codes are valid. The MAC-II approach has multiple anti-hallucination tatics built in — it focuses on very small steps, it validates the codes multiple times and it asks the models for reasoning. These methods will help, but the only true solution with the current state of LLMs is to have a human-in-the-loop system like I described above.
